...
import { MeetingsServiceModule } from './meetings/meetings-service.module';
import { commonAppBootstrap } from './common-setup';
import ConfigService from './config/config.service';
import { isBooleanStringTrue } from './config/env.validation';

async function bootstrap() {
  const app = await NestFactory.create<NestExpressApplication>(MeetingsServiceModule);
  ...
  if (nodeEnv === 'development') {
    const config = new DocumentBuilder()
      .setTitle('CabbageMeet – Meetings Service')
      .setDescription('Meetings microservice API')
      .setVersion('1.0')
      .build();
